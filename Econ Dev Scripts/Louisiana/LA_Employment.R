#Employment Data
# Last edited by Rhea Cong, 11/20/2024
...
region_counties <- county_cbsa %>% filter(CBSA.Title == region_name)
...
LA_counties <- us_counties %>% filter(full==state_name)

fossil_map_data <- left_join(LA_counties, fossil_emp_map, by = c("fips" = "FIPS"))
...
hachman_indices_county <- location_quotients_county %>%
  filter(state==states_simple$fips[states_simple$full==state_name]) %>%
...
soc_la <- soc_data %>% filter(AREA_TITLE == state_name)
          
# Gulf states
 gulf_states <- states_simple %>%
filter(abbr %in% c("LA", "OK", "TX", "AR", "MS", "AL","TN", "FL", "KY"))
soc_gulf <- soc_data %>% 
  filter(AREA_TITLE %in% gulf_states$full) %>%
  group_by(AREA_TITLE) %>%
...
soc_gulf <- soc_gulf %>% filter(OCC_CODE %in% ccia_soc)
soc_gulf$TOT_EMP[116] <- 0
soc_gulf$EMP_PRSE[116] <- 0
soc_gulf_lq <- soc_gulf %>%
  select(AREA_TITLE, OCC_CODE, OCC_TITLE, TOT_EMP, LOC_QUOTIENT, state_emp_percent) %>%
  ungroup() %>%
  group_by(OCC_TITLE) %>%
  mutate(soc_avg_share_Gulf = sum(state_emp_percent)/6)
          
soc_gulf_lq$Gulf_LQ <- soc_gulf_lq$state_emp_percent/soc_gulf_lq$soc_avg_share_Gulf
write.csv(soc_gulf_lq, file="./LA/CCIA_soc_gulf.csv")
write.csv(soc_gulf_lq %>% filter(AREA_TITLE == state_name), file="./LA/LA_soc_gulf.csv")
...
soc_la$TOT_EMP[which(soc_la$TOT_EMP == "**")]  <- NA                                                             
soc_la$STATE_SHARE <- as.numeric(soc_la$TOT_EMP)/1851860 # state share of employment in occupation
LQ_county <- left_join(county_soc, soc_la %>% select(OCC_CODE, OCC_TITLE, TOT_EMP, LOC_QUOTIENT, STATE_SHARE), by = c("Occ. \nCode" = "OCC_CODE")) 
LQ_county$region_share <- as.numeric(LQ_county$`2025\nProjected`)/emp_2025
LQ_county$region_lq <- LQ_county$region_share/LQ_county$STATE_SHARE
LQ_county$national_lq <- LQ_county$region_share/(LQ_county$STATE_SHARE/as.numeric(LQ_county$LOC_QUOTIENT))

write.csv(LQ_county, file="./DataWrapper/LA_county_workforce")
