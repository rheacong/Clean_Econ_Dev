# Clean Investment Monitor for LA
# Last edited by Rhea Cong, 11/11/24
...
# investments at EA and MSA levels
...
investment_year_segment<-investment %>%
  ...
  mutate(Segment = recode(Segment, "energyandindustry" = "Energy & Industry",
                          ...
                          "retail" = "Retail")) %>%
  pivot_wider(names_from=quarter,values_from=Estimated_Actual_Quarterly_Expenditure) #wide format for Datawrapper
...
state_investment_year_dw <- states_investment_year %>%
  filter(State==state_abbreviation, year != "2024") %>%
  ungroup() %>%
  select(-State, -real_gdp, -inv_gdp) %>% 
  pivot_wider(names_from = year, values_from = Value) %>%
  write.csv(file="./DataWrapper/LA_state_investment_yearly")
...
ggsave(paste0(output_folder,"/state_yearly_inv.png"),plot=plot_yearly_inv,width=8,height=6,units="in",dpi=300)


# Plot just the retail
states_investment_year_retail <- investment %>%
  mutate(industry=ifelse(Segment=="Manufacturing",paste0(Technology," Manufacturing"),Technology)) %>%
...
plot_yearly_retail_inv<-ggplot(data=states_investment_year_retail %>%
                                 filter(State==state_abbreviation, year!="2024") %>% # Incomplete data for 2024
                                 select(State,year,Value,industry),aes(x=year,y=Value,fill=industry)) +
  geom_col(position='stack') +
  labs(title=paste("Clean Energy Retail Investment in", state_name, "since 2018"), 
       x="Year", y="Investment Value, Millions USD", fill="Industry") +
  theme_classic()+
  scale_fill_manual(values = rmi_palette)+
  scale_y_continuous(expand=c(0,0))

ggsave(paste0(output_folder,"/state_yearly_retail_inv.png"),plot=plot_yearly_retail_inv,width=8,height=6,units="in",dpi=300)
...
inv_spec_map<-ggplot() +
  geom_sf(data = state_map_data, aes(fill = lq_bin), color = "white") +
...
ggsave(paste0(output_folder,"/state_inv_spec_map.png"),plot=inv_spec_map,width=8,height=6,units="in",dpi=300)
...
write.csv(state_man_wide, file = "./DataWrapper/gulf_investments.csv") # Change this for region of interest
...
facilities_state <- facilities %>%
  ...
  write.csv('./Data/facilities_man_state.csv')
...
facilities_status<-facilities %>%
  group_by(Current_Facility_Status) %>%
  summarize_at(vars(Total_Facility_CAPEX_Estimated),sum,na.rm=T) %>%
  mutate(Total_Facility_CAPEX_Estimated=round(Total_Facility_CAPEX_Estimated/1000,2)) %>%
...
facilities_status_tech<-facilities %>%
  ...
  filter(Current_Facility_Status != "C" & year>2021) %>%
  group_by(Current_Facility_Status, Technology) %>%
  summarize_at(vars(Total_Facility_CAPEX_Estimated),sum,na.rm=T) %>%
  mutate(Total_Facility_CAPEX_Estimated=round(Total_Facility_CAPEX_Estimated/1000,2)) %>%
  pivot_wider(names_from=Current_Facility_Status,values_from=Total_Facility_CAPEX_Estimated) %>%
...
facilities_state_sector<-facilities %>%
  ...
  filter(year>2020) %>%
  left_join(states_simple,by=c("State"="abbr")) %>%
  filter(Segment=="Manufacturing" & Current_Facility_Status != "C") %>%
  group_by(region,State,full,Technology) %>%
  summarize_at(vars(Total_Facility_CAPEX_Estimated),sum,na.rm=T)%>%
  group_by(region,State,full) %>%
  mutate(share=Total_Facility_CAPEX_Estimated/sum(Total_Facility_CAPEX_Estimated)) %>%
  arrange(desc(Total_Facility_CAPEX_Estimated))

facilities_state_status<-facilities %>%
  left_join(states_simple,by=c("State"="abbr")) %>%
  group_by(region,State,full,Segment,Current_Facility_Status) %>%
  summarize_at(vars(Total_Facility_CAPEX_Estimated),sum,na.rm=T) %>%
  pivot_wider(names_from=Current_Facility_Status,values_from=Total_Facility_CAPEX_Estimated) %>%
...
batteries_sc <- supply_chains %>%
  ...
  select(-`1`,-`0`) %>%
  arrange(desc(`post_ira`)) %>%
  write.csv('./Data/batteries_sc.csv')
...
#county_stats$geoid<-paste0(county_stats$STATE,county_stats$county) # where is this variable from
...
DW_facilities_msa_tech <- msa_tech_year_facilities %>% 
  group_by(Technology) %>%
  summarise(Total_CAPEX = sum(Total_Facility_CAPEX_Estimated))
...
ggsave(file.path(output_folder, paste0(state_abbreviation,"_msa_tech_yearly_plot", ".png")),  
       plot = plot_yearly_msa_facilities,
...
facilities_EA_total <- facilities %>%
  mutate(announce_year=substr(Announcement_Date,1,4)) %>%
  filter(Current_Facility_Status != "C" & announce_year %in% c("2022","2023")) %>%
  right_join(EAs,by=c("county_2020_geoid"="fips")) %>%
  group_by(`EA Name`) %>%
  summarize_at(vars(Total_Facility_CAPEX_Estimated),sum,na.rm=T)%>%
  ungroup() %>%
  mutate(Percentile = percent_rank(-Total_Facility_CAPEX_Estimated),
...
state_estimates_tech <- state_estimates2 %>%
  group_by(Category) %>%
  summarise(Federal_Investment_M2022 = sum(`Federal Investment (millions 2022 USD)`)) %>%
  write.csv(file="./DataWrapper/Fed_inv_tech")

ggplot(data=state_estimates_tech) +
  geom_col(aes(x=reorder(Category, -Federal_Investment_M2022),y=Federal_Investment_M2022),position="stack")
...
state_map_data <- state_map_data%>%
  mutate(invcap_bin=cut(Federal.Investment..2022.USD.per.capita.,breaks=c(0,50,100,250,500,1000, Inf),labels=c("0-$50","$50-100","$100-250","$250-500","$500-1000","$100+")))

#Write csv for DataWrapper
write.csv(state_map_data, file = "./DataWrapper/state_map_investments")
...
# Plotting the US state map with investment data
### can't figure out this x,y situation
...
ggsave(file.path(output_folder, paste0(state_abbreviation,"_region_states_ira_plot", ".png")),  
       plot = state_ira_plot,
       width = 15,   # Width of the plot in inches
       height = 8,   # Height of the plot in inches
       dpi = 300)

write.csv(state_ira %>% select(Category, `Federal Investment (millions 2022 USD)`, State.y) %>%
            pivot_wider(names_from = "State.y", values_from = "Federal Investment (millions 2022 USD)"), 
          file=paste0("./DataWrapper/", state_abbreviation, "_fed_ira_division"))

# Updated total investments across US for DataWrapper
dw_graph <- facilities %>% mutate(date=as.Date(Announcement_Date),
                                  year=substr(date,1,4),
                                  post_IRA = ifelse(date>"2022-08-15",1,0)) %>% 
  filter(post_IRA == "1") %>% 
  select(Company,Segment, Technology,State, Current_Facility_Status, Address, Latitude, Longitude, Announcement_Date, Total_Facility_CAPEX_Estimated) %>%
  filter(Segment == "Manufacturing")

write.csv(dw_graph, file="./DataWrapper/updated_CIM_graph")
