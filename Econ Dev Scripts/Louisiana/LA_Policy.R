...
tax_inv_state<-read.csv('./Data/clean_investment_monitor_q1_24/tax_investment_by_state.csv',skip=2)
...
write.csv(state_estimates2,"./Data/IRA_taxcredits_estimate.csv")
...
xchange <- read.csv("./Data/xchange.csv")
xchange_pol_index <- read.csv("./Data/xchange_climate_policy_index.csv")

xchange_label<-xchange_pol_index %>%
  #filter(region %in% region_abbrv$region) %>%
  rename(value=climate_policy_index) %>%
  mutate(label=paste(xchange_pol_index$abbr,"(Climate Policy Score =",round(value*100,1),")")) %>%
  ungroup() %>%
  rename(total=value, State=abbr) %>%
...
state_totals <- xchange %>%
  filter(abbr %in% region_interest$abbr) %>%
  group_by(State) %>%
  summarize(total = sum(value)) 
#%>% left_join(xchange_label %>% select(State,label), by = "State")

policy_radial<-ggplot(xchange %>% filter(abbr %in% region_interest$abbr,
                                         Topic != "total_climate") %>%
  ...
  geom_text(data = state_totals, aes(x = State, y = total+3, label = State), 
            vjust = -0.5, hjust = 0.5, size = 3, color = "black",inherit.aes=F) +
  labs(title = "Climate Policy Strength in the Gulf Coast",
...
# XChange ratings for DataWrapper
xchange_state <- xchange %>% filter(State == state_name) %>% select(Policy, value, Topic)
write.csv(xchange_state %>% pivot_wider(names_from = "Topic", values_from = "value"), file=paste0("./DataWrapper/", state_abbreviation,"_policy_scores.csv"))

#Economic Development Incentives

#Good Jobs First Data
gjf<- read.csv("./Data/gjf_complete.csv")
