#Electricity Sector
# Last edited by Rhea Cong, 11/13/2024
...
### OUTPUTS
# graph of generation capacity additions at state level since 2012
# line graph for state comparison of RE growth, indexed to 2022
# calculations of renewable share within region and state
# generation emissions map
# industrial electricity prices and expenditure graphs, projected to 2050
# electricity consumption by MSA and by source
...
region_id_state <- region_id %>%
  filter(abbr==state_abbreviation)

# Set the states you want to compare the state to
region_interest <- states_simple %>%
  filter(abbr %in% c("LA", "OK", "TX", "AR", "MS", "AL","TN", "FL", "KY"))

#Filter for specific counties from US Map package
region_counties_map<-us_counties %>%
...
# Write regions of interest for DataWrapper
states_comp <- region_interest$full
ren_gen_data <- states_rengen %>% filter(full %in% states_comp) %>%
  select(full, `Operating Year`, cap_index_18) %>%
  pivot_wider(names_from = "full", values_from = "cap_index_18")
write.csv(ren_gen_data, file=paste0("./DataWrapper/",state_abbreviation, "_state_rengen_comp"))

cum_ren_gen_data <- states_rengen %>% filter(full %in% states_comp) %>%
...
write.csv(state_gen_cap_total, file=paste0(output_folder, "/",state_abbreviation, "_gen_cap_total.csv"))
...
sbrgn_sf_transformed <- st_transform(shapefile, crs = st_crs(region_counties_map))
...
matched <- st_join(sbrgn_sf_transformed, region_counties_map, join = st_intersects)
sf::sf_use_s2(TRUE)
matched<-as.data.frame(matched)
matched <- matched %>%
  select(ID,NAME,STATE,CNTRL_AREA,HOLDING_CO,NET_GEN,CUSTOMERS,fips) %>%
...
utilities$ID <- as.numeric(utilities$ID)
epa_plnt$OPRCODE <- as.numeric(epa_plnt$OPRCODE)

#Join with retail service areas to get county-level estimates
epa_plnt$fips <- as.numeric(paste0(epa_plnt$FIPSST, epa_plnt$FIPSCNTY))
epa_retail_area <- epa_plnt %>%
  select(PNAME,OPRNAME,OPRCODE,UTLSRVNM,SRNAME,PLGENATN,	PLGENATR,	PLGENAWI,PLGENASO,PLGENATH,	PLGENACY,	PLGENACN,PLCO2AN, PLCO2RTA, FIPSST,FIPSCNTY,fips) %>%
  inner_join(utilities,by=c("OPRCODE"="ID","fips"))

region_retail_area <- epa_retail_area %>%
  filter(fips %in% region_id$fips) %>%
  distinct(STATE,`EA Name`,CBSA.Title,fips,OPRNAME,UTLSRVNM,SRNAME)
...
opr_eas <- matched %>%
  ...
  distinct(`EA Name`,ID,NAME,NET_GEN,CUSTOMERS) %>%
  mutate(ID = as.numeric(ID)) %>%
...
write.csv(EA_renshare,"./LA/EA_renshare.csv")
multi_region_id <- EAs %>% 
...
county_map_data <- us_counties %>%
  left_join(EAs %>% select(`EA Name`,FIPS),by=c("fips"="FIPS"))%>%
  left_join(EA_renshare,by=c("EA Name"="EA Name")) %>%
  filter(full %in% c(region_interest$full, state_name)) %>%
  mutate(region_id=ifelse(fips %in% region_id_state$fips,1,0.7))
...
region_map <- us_states %>% filter(region %in% county_map_data$full)
...
ren_gen_map<-ggplot() +
  ...
  geom_sf(data = us_map(regions = "state") %>% filter(full==state_name), color = "black", fill = NA, size = 100, alpha=0.5) +
  #geom_text(data = county_labels, size = 2, color = "black", fontface = "bold") +
  labs(title = paste("Electricity Generation Emissions in", "Gulf","States"), #Change to the region name you want
       subtitle = "",
...
datawrapper_county_gen_emissions <- county_map_data %>%
  as.data.frame() %>%
  select(fips, full, county, `EA Name`, total_gen, ren_mix, em_rate) 

write.csv(datawrapper_county_gen_emissions, file = "./DataWrapper/elec_gen_emissions_great_lakes")
...
states_comp_seds_elec_price <- seds_elec_pric_ind %>% filter(State %in% region_interest$full, MSN=="ESICD") %>%
...
# Plot industrial electricity expenditure/GDP
industrial_exp_gdp_plot<-ggplot() +
  ...
  scale_size_identity()+
  labs(title=paste0("Industrial Electricity Expenditure/GDP in the ","Gulf", " Division since 2002"), 
       subtitle="",
       y="Expenditure/GDP",
       x="Year",
       caption="Source: EIA") +
...
industrial_net_exp_plot<-ggplot() +
  ...
  scale_size_identity()+
  labs(title=paste0("Electricity Net Export Expenditure in the ","Gulf", " Division since 2002"), 
       subtitle="",
       y="$ Millions",
       x="Year",
       caption="Source: EIA") +
...
write.csv(seds_ren_prod, file = paste0(output_folder, state_abbreviation, "_seds_red_prod"))
...
# Electricity consumption breakdown in state of interest
state_county_eleccons <- county_eleccons %>%
  filter(State.Name ==state_name) %>%
  group_by(CBSA.Title) %>%
  summarise(eleccons = sum(Consumption.MMBtu)) %>%
  mutate(percent = eleccons/sum(eleccons))

state_county_eleccons$CBSA.Title <- ifelse(state_county_eleccons$percent < 0.03, "Other", as.character(state_county_eleccons$CBSA.Title)) # only keep the MSA names with large consumption, 0.3 threshold can be changed

state_county_eleccons <- state_county_eleccons %>%
  group_by(CBSA.Title) %>%
  summarise(eleccons = sum(eleccons), percent = sum(percent))

write.csv(state_county_eleccons, file=paste0("./DataWrapper/", state_abbreviation, "_eleccons_msa"))

# Create GeoJSON for emissions graph
# Vector of FIPS codes or state abbreviations for the states you want
states <- c("17", "18", "26", "27", "39", "55") # Great Lakes: Illinois, Indiana, Michigan, Ohio, Wisconsin, Minnesota

# Download county data for those states
counties <- counties(state = states, cb = TRUE, year = 2023, class = "sf")
filtered_counties <- counties[counties$STATEFP %in% states, ]

geojson_write(filtered_counties, file = "./DataWrapper/GL_counties_states.geojson")
