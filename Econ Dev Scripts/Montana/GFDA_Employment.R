...
region_cbp_2d <- cbp21_2d %>%
  filter(abbr.x==state_abbreviation) %>%
...
fossil_map_data <- left_join(state_counties, fossil_emp_map, by = c("fips" = "FIPS"))
...
emp_proportions <- cbp21_2d %>%
  left_join(total_emp %>% select(state,county,full.x,,EMP,FIPS), by = c("full.x"="full.x","state"="state","county"="county","FIPS"="FIPS"), suffix = c("", "_total")) %>%
  select(FIPS,state,county,full.x,NAICS2017,naics_desc,EMP,EMP_total) %>%
  group_by(FIPS,state,county,full.x,NAICS2017,naics_desc) %>%
  mutate(emp_share=EMP/EMP_total) %>%
  filter(!is.na(full.x)) %>%
...
for (area_code in state_gdp$GeoFips) {
  current_data <- blsAPI::blsQCEW(method = 'Area',
                          year = '2023',
                          quarter = '3',
                          area = area_code)
...
all_industries<-blsAPI::blsQCEW(method = 'Industry',
                        year = '2023',
                        quarter = '3',
                        industry = '10')
