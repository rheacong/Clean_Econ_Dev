...
# Last edited by Rhea Cong, 07/03/2024
...
#Direct Emitters
emit<-read_excel('./Data/direct_emitters.xlsx')
...
# Plot the bubble map
state_emitters_map <- ggmap(base_map, darken = c(0.5, "white")) +
...
ggsave(file.path(output_folder, paste0(state_abbreviation,"_emitters_map", ".png")), 
       plot = state_emitters_map,
...
state_directemit_naics <- emit %>%
  ...
  left_join(naics2017 %>% 
              mutate("2017 NAICS Code"=as.integer(`2017 NAICS US   Code`),
                     NAICS_name=`2017 NAICS US Title`) %>%
              select("2017 NAICS Code","NAICS_name"),by=c("Primary NAICS Code"="2017 NAICS Code")) %>%
...
ggsave(paste0(output_folder,"/",state_abbreviation,"_directemit_lq_plot.png"),plot=directemit_lq_plot,width=10,height=6,units="in",dpi=300)
...
write.csv(fed_rdd, file="./DataWrapper/federal_rdd_exp.csv")
