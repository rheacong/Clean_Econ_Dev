...
### is this supposed to plot top producers, not specific to state of interest?
...
### ERROR where do you read in nzap?
...
tax_rev <- read_excel('./Data/rff_taxrev.xlsx', sheet = 2)
...
## these all have different y axes, which makes them confusing to compare
...
ggsave(file.path(output_folder, paste0(state_abbreviation,"_plot_taxrev_states", ".png")), 
       plot = plot_taxrev_state,
       width = 8,   # Width of the plot in inches
       height = 8,   # Height of the plot in inches
       dpi = 300)

# plot for just state of interest
state_taxrev <- ggplot(data=state_rev %>% filter(State == state_abbreviation),aes(x=`Fiscal Year`,y=amount,group=energy,fill=energy)) +
  geom_col() +
  facet_wrap(~State, scales="free_y") +  # Adding faceting to create separate plots for each scenario
  scale_fill_manual(values = expanded_palette, name="Energy")+
  labs(title=paste("Energy Tax Revenue in high fossil-producing states"),
...
ggsave(file.path(output_folder, paste0(state_abbreviation,"_plot_taxrev", ".png")), 
       plot = state_taxrev,
       width = 8,   # Width of the plot in inches
       height = 8,   # Height of the plot in inches
       dpi = 300)
