# Clean Investment Monitor for GFDA
# Last edited by Rhea Cong, 08/16/24
...
state_division <- as.vector(census_divisions %>%
...
# Total state investments since IRA
state_inv <- states_investment_quarterly %>% filter(State==state_abbreviation) %>%
  select(State,year_quarter,Value,industry)

state_inv$year_quarter <- as.Date(MT_inv$year_quarter)

state_inv_ira <- state_inv %>% filter(year_quarter > as.Date("2022-08-01")) %>%
  group_by(industry) %>%
  summarise(total_inv = sum(Value))
...
ggsave(paste0(output_folder,state_abbreviation,"_state_investment_gdp.png"), plot = plot_invgdp, width = 8, height = 6, dpi = 300)
...
ggsave(paste0(output_folder,region_abbrv$region,"_manufacturing_since_IRA.png"), plot = plot_manufacturing, width = 8, height = 6, dpi = 300)
...
facilities$fips <- as.numeric(facilities_with_counties$GEOID)
...
facilities<-left_join(facilities,county_cbsa,by="fips")
...
state_ira_plot3<-ggplot(data=state_ira) +
  geom_col(aes(x=reorder(State,-`Federal Investment (millions 2022 USD)`),y=`Federal Investment (millions 2022 USD)`,fill=Category),position="stack") +
  #coord_flip() +
  scale_fill_manual(values=rmi_palette) +
  labs(title = paste("Federal IRA Investment in the ",division_of_interest$Division," Division by Tax Credit"),
...
# Summary of investment across regions
regions_summ <- state_estimates2 %>%
  left_join(states_simple, by = c("State"="abbr")) %>%
  ungroup() %>%
  group_by(region) %>%
  summarise(fedinvsum22 = sum(`Federal Investment (millions 2022 USD)`))
