...
write.csv(state_county_eleccons, file="./DataWrapper/MN_eleccons")
...
MSA_gen <- op_gen_with_county %>%
  mutate(fips=as.numeric(GEOID)) %>%
  inner_join(MSAs,by=c("fips"="fips")) %>%
  filter(Status=="(OP) Operating") %>% #Only Operating Plants
  left_join(states_simple, by=c("Plant State"="abbr")) %>%
...
gen_cap_share$Name <- c("State", "Minnesota-St. Paul-Bloomington, MN-WI")
...
### ERROR: Minnesota has 0 fossil fuels
