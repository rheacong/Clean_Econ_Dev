# Clean Investment Monitor for MN
# Last edited by Rhea Cong, 08/06/24
...
write.csv(state_man_wide, file = "./DataWrapper/great_lakes_investments.csv") # Change this for region of interest
...
facilities<-left_join(facilities,county_cbsa %>% mutate(fips=as.character(fips)),by="fips")
...
state_ira <- state_estimates2 %>%
  ...
  filter(State.y %in% c(division_of_interest$State, state_name)) 
...
this_state_ira <- state_ira %>%
  filter(State == state_abbreviation)

write.csv(this_state_ira, file = paste0("./DataWrapper/",state_abbreviation, "_ira_clean_inv.csv"))
