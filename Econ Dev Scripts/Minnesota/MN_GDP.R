...
### INPUTS
# Bureau of Economic Analysis SAGDP by NAICS code at state level

### OUTPUTS
# Calculate LQ and Hachman Index with GDP
# Graph state specializations in transition sectors
...
# Real GDP is in millions of chained 2017 dollars. Industry detail is based on the 2017 North American Industry Classification System (NAICS)
...
test <- left_join(MSA_cbp_2d, gdp_ind2 %>% filter(GeoName=="Minnesota"), by=c("code"="IndustryClassification"))
...
top_sectors_plot<-ggplot(top_sectors, aes(x = reorder(sector, LQ), y = LQ, fill = (GeoName.x))) +
  geom_bar(stat = "identity", position = position_dodge(),
           aes(alpha = ifelse(state == "0", 0.3, ifelse(state == "1", 1, 1)))) +
