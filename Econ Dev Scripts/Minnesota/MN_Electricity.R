...
plot_elec_2020index<-ggplot(data=states_rengen %>%
  ...
  geom_line(data = subset(states_rengen%>%filter(region == region_abbrv$region,), full == state_name), size = 2) + # Note that region is specific to project
  scale_size_identity() +
...
states_comp <- c(census_divisions$State[census_divisions$Division=="East North Central"], "Minnesota")
...
write.csv(ren_gen_data, file="./DataWrapper/MN_state_rengen_comp")
...
region_utility <- utilities %>%
  filter(GEOID %in% region_id_state$fips) 
...
county_map_data <- us_counties %>%
  ...
  filter(full %in% c(division_of_interest$State, state_name)) %>%
...
ren_gen_map<-ggplot() +
  ...
  labs(title = paste("Electricity Generation Emissions in", "Great Lakes","States"), #Change to the region name you want
       subtitle = "",
...
states_comp_seds_elec_price <- seds_elec_pric_ind %>% filter(State %in% states_comp, MSN=="ESICD") %>%
